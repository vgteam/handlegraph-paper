...
ggplot(subset(bdsg.prof,graph.model=="vg"), aes(y=graph.avg.degree, x=graph.seq.length, color=graph.avg.path.depth, shape=graph.avg.path.depth==0)) + geom_point(alpha=I(1/3)) + scale_x_log10("graph sequence length (bp)") + scale_y_continuous("average node degree") + theme_bw() + theme_bw() + scale_shape_discrete("h
...
ggplot(subset(bdsg.prof.df.iter, value>0), aes(x=graph.seq.length, y=value, color=graph.model)) + geom_point(size=0.5, alpha=I(1/2)) + scale_x_log10("graph sequence length (bp)") + scale_y_log10("") + scale_color_discrete("model") + facet_grid(name ~ ., scales = "free_y") + theme_bw()
...
ggplot(subset(bdsg.prof.df.mem, value>0), aes(x=graph.seq.length, y=value, color=graph.model)) + geom_point(size=0.5, alpha=I(1/2)) + scale_x_log10("graph sequence length (bp)") + scale_y_log10("") + scale_color_discrete("model") + facet_grid(name ~ ., scales = "free_y") + theme_bw()
