...
The \textsc{HandleGraph} model describes a bidirected sequence graph in terms of four fundamental entities:
...
The documentation of this interface at \url{https://pangenome.github.io/odgi/odgipy.html} can be used to understand the full scope of the \textsc{HandleGraph} API.
...
\begin{figure}
	...
	\end{center}
	\caption{{\label{fig:hgsvc} \textbf{Performance on a graph of structural variants from the HGSVC.} All four new graph implementations compare favorably to \textsc{vg}. PackedGraph tends to be the most memory efficient, HashGraph tends to be the fastest, and ODGI is balanced in between. \textsc{xg} provides good performance on both memory usage and speed, but it is static.}}
\end{figure}
...
\begin{figure} %[t]{1.0\textwidth}
  ...
  \caption{
    ...
    Most graphs contain edges, with the exception of one large graph built from a set of linear sequences (bottom right outlier), which has no edges.
    In general, graphs in the collection have a path set, but the path depth (number of paths touching each node) may be very low.
    }
\end{figure}
...
\begin{figure}
  ...
  \caption{
    \label{fig:prof1}
    \textbf{Construction and loading performance of models on the graph collection.} 
...
\begin{figure}
  ...
  \caption{
    \label{fig:prof2}
    \textbf{Iteration performance of models on the graph collection.} 
    }
\end{figure}
