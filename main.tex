...
Thus, even though \textsc{HashGraph} still has relatively high memory requirements, it can still be useful in high memory compute environments or for small sequence graphs (or subgraphs of larger graphs).
...
Each hash table entry contains the sequence, an adjacency list of the edges in a STL vectors, and a vector indicating the path steps that the node can be found on.
...
This encoding is split between graph topology and paths, which is important for achieving a balance of runtime performance and memory usage on real-world graphs with large path sets.
...
The bit-width of these vectors is chosen dynamically, starting with a bit-width of 1 and  then expanding the wideth and reallocating the vector whenever an edit operation introduces an integer that is too large to be represented with the current width.
...
The graph topology is represented using an adjacency list, which is efficient for sparse graphs.
Since the implementation must provide efficient edits both to these adjacencies and to stored paths, both of them are represented using linked lists.
This allows $O(1)$ modifications even at internal positions in the list.
The linked list is expressed using the same bit-compressed integer vector, where pointers are created by treating some integer entries as indexes into the vector itself.
This encoding also guarantees the technical condition that the $i$-th entry in the vector is $O(i)$.
...
Specifically, the graph consists of the GRCh38 primary scaffolds and 72,485 indel variants ranging in size from 50bp to 76kbp.
