...
\title[short Title]{\hl{Efficient} dynamic variation graphs}
...
For instance, the speed and memory usage is up to an order of magnitude better than the prior graph implementation in the \hl{\textsc{VG}} toolkit, which has now transitioned to using \texttt{libbdsg}'s implementations.\\
...
\hl{As increasingly many individuals have been sequenced from certain species, the field of \emph{computational pangenomics} has emerged to analyze whole populations of genomes rather than individual genomes} \citep{computational2016computational}.
...
Unlike conventional string-based representations, graph data structures \hl{can represent} genomic variation like substitutions, insertions, deletions, and other more complex genomic events.\hl{ }

Graph-based data structures\hl{ }present new computational challenges.
In addition to sequence, genome graphs must represent topology.
Given the size of many genomes\hl{}, this can be quite demanding on computer memory.\hl{ }
However, the total information content \hl{in a genome graph} is only incrementally more than the sequences of the pangenome.
This suggests that significant memory savings should be possible.
\hl{There is also significant impetus to make the graph data structures computationally efficient, since they are frequently the core data structure in pangenomics applications.}

Early versions of the variation graph toolkit (\hl{\textsc{VG}}) \citep{Garrison_2018} have provided a cautionary tale of a na\"ive implementation.
\hl{\textsc{VG}} used full-width machine words as identifiers for graph elements, and stored the elements and graph topology in a set of hash tables.
Loading the 1000 Genomes Project's variant set into the \hl{\textsc{VG}} toolkit used to consume more than 300~GB of memory, which is $\sim$30 times as large as the serialized representation \citep{Garrison_2019}.

Although \hl{\textsc{VG}} provided a memory-efficient\hl{ }representation of the graph (\hl{\textsc{XG}}) that could be used during read mapping and variant calling, \hl{this} representation did not allow for dynamic updates to the graph.
\hl{The dynamic implementation remained necessary for graph-modifying steps of \textsc{VG} pipelines, such as the original construction of the graph and augmenting the graph with novel variants.
Some pipelines could be made feasible by breaking} large graphs into connected components.
\hl{However, this strategy reduces efficiency, and it is untenable for pangenome graphs that consist of a single component.}

To overcome this limitation, we have developed three new graph genome data structures that are dynamic \hl{(allowing efficient updates and edits)} and also memory-efficient \hl{for real world genome graphs}.
Here, we compare the performance of these data structures to \hl{the original \textsc{VG} representation and to \textsc{XG}}, using a diverse collection of genome graphs obtained during our work in graphical pangenomics.
...
The \textsc{VG} toolkit has been refactored to use this API as its default means of loading, saving, and manipulating graphs since version 1.22.0\hl{, allowing it to use any of the implementations presented here}. 

%To support reuse by other researchers,
We \hl{have packaged} these implementations behind equivalent C++ and Python APIs in \texttt{libbdsg}.
This software library will reduce the need for individual research groups to continually reimplement these core data structures \hl{and ease the development of algorithms that manipulate} large, complex pangenome graphs.
\hl{Moreover, the reduction in memory requirements makes it possible to move workloads that would otherwise need specialized high-memory machines onto cheaper ones that often also have more processing power (for example, from Amazon's r4 instances to c5 instances).
...
\hl{Nodes can be traversed either from left to right, which is interpreted as the forward strand of the sequence, or from right to left, which is interpreted as the reverse complement.
This provides a natural means to encode DNA strandedness.}

\hl{Longer sequences can be formed by concatenating the sequences of multiple adjacent nodes together.
These nodes form a path, which is defined as a list of oriented nodes (either forward or reverse), such that the graph contains an edge between the adjacent sides of each pair of subsequent oriented nodes in the list}\footnote{\hl{Unlike the usage in many graph theoretic contexts, we do not intend the term path to indicate that these nodes must be distinct.}}.
Some paths correspond to sequences of interest, such as reference genomes or annotations of the reference.
Because paths like these are so frequently important in practice, our graph formalism also includes \hl{a set of paths along with the graph's node and edges}.

\subsection{The \textsc{HandleGraph} interface}

The \texttt{libhandlegraph} library describes an\hl{ }interface that exposes basic operations on our sequence graph data model.
...
\item \emph{Strands} \hl{represent} one strand of a node's DNA sequence.
\item \emph{Edges} link pairs of strands, in order.
\item \emph{Paths} represent sequences \hl{of interest as paths} through the graph.
...
Instead, \hl{they are accessed via \emph{handles}, which are references modeled after the concept of file handles.
...
\hl{Since the actual contents of a handle are unspecified, this benefit is achieved while simultaneously maintaining flexibility in the implementation.
...
\begin{figure}
	...
	\caption{{\label{fig:graph}
        ...
        Edges run from the strand at their beginning \hl{to that at their end}, as indicated by the arrowhead.
        Bottom: an illustration of four \emph{paths}.
        \hl{Each has a name, and can be referenced by a handle, which are omitted for brevity}.
...
Two implementations, \hl{\textsc{VG} and \textsc{XG}}, have been described previously \citep{Garrison_2018,Garrison_2019}.
The others are combined in the \texttt{libbdsg} library (\url{https://github.com/vgteam/libbdsg}), which provides three concrete implementations: \textsc{HashGraph}, \hl{\textsc{ODGI}}, and \textsc{PackedGraph}.
Each implementation represents a different tradeoff in terms of speed, memory use, and capabilities.
All of the implementations except \hl{\textsc{XG}} are dynamic.
\hl{They support efficient addition and deletion of nodes, edges, paths, and steps, as well as some specialized methods such as splitting a node into multiple shorter nodes.
Table }\ref{table:comparison} \hl{provides a high level summary of the differences between the \texttt{libbdsg} implementations.}
...
\subsubsection{\hl{\textsc{VG}}}

We have extended the \hl{graph representation} in \textsc{VG}, previously described in \citep{Garrison_2018}, to match the \textsc{HandleGraph} API.
The backing data structures used\hl{ }remain the same.
...
Its inefficiency, caused by unnecessary overheads and data duplication, has \hl{resulted in significant difficulties for groups working with \textsc{VG}.}
...
\subsubsection{\hl{\textsc{XG}}}

\hl{\textsc{XG}} was initially developed in response to the memory and runtime costs of \hl{\textsc{VG}}	, which prevent its application to large graphs.
...
Unlike other \textsc{HandleGraph} implementations, \hl{\textsc{XG}} is a static graph index.
...
\hl{\textsc{HashGraph} is a relatively simple encoding, which is largely similar to the original \textsc{VG} graph.
...
The hash table it uses is a drop-in replacement for the equivalent standard library (STL) data structure, and has been shown to outperform it in empirical evaluations} \citep{brehm2019hash}\hl{.
...
\subsubsection{\hl{\textsc{ODGI}}}

\hl{\textsc{ODGI}} (Optimized Dynamic Graph Implementation) is based on a node-centric encoding that is designed to improve cache efficiency when traversing or modifying the graph.
This encoding is split between graph topology and paths, which is important for achieving a balance of runtime performance and memory usage on \hl{graphs with large path sets}.
\hl{It uses delta encoding of edges and path steps to reduce the cost of representing graphs with local partial order and sparsity, both of which are common features of pangenome graphs.
\textsc{ODGI} is the default data model of the \textsc{ODGI} toolkit (}\url{https://github.com/vgteam/odgi}\hl{), which provides high-level algorithms for graph manipulation and interrogation that are designed to work at the scale of large pangenomes.

In \textsc{ODGI}, each node }$\mathcal{N} = ( \mathcal{B}, \mathcal{P} )$\hl{ is represented by a structure that contains a byte array }$\mathcal{B} = (\mathcal{Q}, \mathcal{E})$\hl{ encoding its sequence and associated edges, and a compressed integer vector }$\mathcal{P} = \mathcal{S}_1 \ldots \mathcal{S}_{s}$\hl{ describing the path steps that traverse it.
The full graph model is simply an array of these node records }$\mathcal{G} = \mathcal{N}_1 \ldots \mathcal{N}_{|\mathcal{G}|}$\hl{ with some additional data structures to allow for random access of paths by name, and to maintain important statistics about the size of the graph, its node ID space, and its path set.

Each node's sequence }$\mathcal{Q}$\hl{ is stored using a full byte per character at the start of the byte array }$\mathcal{B}$\hl{.
...
The edges that begin or end at the node are recorded in the remainder of }$\mathcal{B}$\hl{, encoded as deltas between the rank of the other end of the edge and the current node.
...
The path steps }$\mathcal{P} = \mathcal{S}_1 \ldots \mathcal{S}_{s}$\hl{ on each node are recorded as a series of records in a dynamic integer vector which is compressed so that only the largest integer entry is stored at full bit-width }\citep{prezza2017framework}\hl{.
...
This path encoding scheme is similar to that used in the dynamic GBWT }\citep{Siren_2019}\hl{, but differs in that the paths are not prefix-sorted.}
...
\hl{The backing data structures are implemented using bit-compressed integer vectors.
...
In the typical case that the value of }$i$\hl{-th entry in the vector is }$O(i)$\hl{, these reallocations have an }$O(1)$\hl{ amortized run time per edit.
...
This pointer encoding also guarantees the technical condition that accessing the }$i$\hl{-th entry is }$O(i)$\hl{.
...
\hl{\textbf{Model}} & \hl{HashGraph} & \hl{ODGI} & \hl{PackedGraph}  \\
\hline
\hl{\textbf{Design goal}} & \hl{Simplicity, speed} & \hl{Memory efficiency} & \hl{Balanced speed/memory} \\
\Xhline{0.1\arrayrulewidth}
\hl{\textbf{Topology data structure}} & \hl{Hash table} & \hl{Single integer vector} & \hl{Several integer vectors} \\
\Xhline{0.1\arrayrulewidth}
\hl{\textbf{Topology compression}} & \hl{None} & \hl{Delta encoding} & \hl{Windowed bit compression} \\
\Xhline{0.1\arrayrulewidth}
\hl{\textbf{Sequence compression}} & \hl{None} & \hl{None} & \hl{Bit compression} \\
\Xhline{0.1\arrayrulewidth}
\hl{\textbf{Pointer encoding}} & \hl{Memory addresses} & \hl{Delta-encoded ranks} & \hl{Vector indexes} \\
\hline
\end{tabular}
\caption{
 \hl{ \textbf{Comparison of features between libbdsg graph implementations.}
...
\hl{Documentation for the two libraries, including the C++ handle graph API, \textsc{HashGraph}, \textsc{ODGI}, and \textsc{PackedGraph}, is available at }\url{https://bdsg.readthedocs.io}\hl{ alongside the documentation for the Python binding.}
...
We measured the core operation performance of the four graph implementations and the graph class from the popular \hl{\textsc{VG}} software (as implemented prior to version 1.22.0).
...
\hl{These access operations are one of the major drivers of run time in pangenomic applications, such as \textsc{VG}'s read mapping algorithm. 
...
The presented results are from a graph describing the structural variants of the Human Genome Structural Variation Consortium \citep{chaisson2019multi}\hl{, which was recently used to genotype structural variants }\citep{hickey2020genotyping}\hl{.
...
\begin{figure}
  ...
  \caption{
    ...
    The same pattern can be seen for \hl{\textsc{VG}}, although the overall performance is worse.
    ...
    The relative path encoding in \hl{\textsc{ODGI}} yields poor performance on path iteration, and node decoding overheads appear to reduce its node iteration performance, but it has good graph topology traversal performance, perhaps due to cache efficiency of the edge encoding.
    \hl{\textsc{XG}} provides excellent iteration performance in all cases.
    }
\end{figure}
...
\textsc{PackedGraph} yields the best memory performance for larger graphs (which are mostly the chromosomes of the 1000 Genomes Project graph), while for the medium-sized graphs in the collection ($\sim$1~Mbp), \hl{\textsc{ODGI}} requires less memory.
...
However, we observe that the hash-based models (\hl{\textsc{VG}} and \textsc{HashGraph}) have very good performance for smaller graphs (in handle and edge enumeration) but decrease in throughput as the graph size increases.
Smaller, less dramatic decreases in performance can be seen for the other implementations.
For path enumeration, the highest-performing methods are \hl{\textsc{XG}} and \textsc{HashGraph} at approximately 10 times faster than \hl{\textsc{ODGI}}, whose relative path storage is costly to traverse.
...
\begin{tabular}{c||r|r|r|r|r}
 & \multicolumn{1}{c|}{Build} & \multicolumn{1}{c|}{Load} & \multicolumn{3}{c}{\hl{Iteration rate (millions)}} \\
...
\hline
vg &   \hl{80.2} &                \hl{77.2}$\ \ $  &             \hl{ 24.6} &                  \hl{2.8} &               \hl{2.9} \\
hg &   \hl{36.7} &                \hl{23.9}$\ \ $  &              \hl{59.5} &                 \hl{18.9} &             \hl{ \textbf{127.2}} \\
og &   \hl{\textbf{30.3}}  &   \hl{13.7}$\ \ $  &              \hl{24.1} &                 \hl{11.5 }&              \hl{8.2} \\
pg &   \hl{37.6} &                 \hl{\textbf{3.80}}  &   \hl{\textbf{63.7}} &   \hl{4.6} &               \hl{ 24.3 }\\
xg &   \hl{54.3} &                \hl{9.31}  &              \hl{ 54.2 }&                 \hl{\textbf{20.5}} & \hl{117.0} \\
\end{tabular}
\caption{
  ...
  \hl{\textsc{VG}} requires~$\sim$~20 times as much memory to load the graphs as \textsc{PackedGraph}, while even the most costly \texttt{libbdsg} model (\textsc{HashGraph}) requires~$\sim$~1/3 as much memory.
  In these graphs, \hl{\textsc{ODGI}} provides the lowest performance for handle iteration.
  However, in all other metrics, \hl{\textsc{VG}} performs much worse than the other models.
}
...
We find that the best-performing method in terms of memory usage is \textsc{PackedGraph}, which consumes around $1/20$th the memory of \hl{\textsc{VG}} per base-pair of graph in the 1000GP set.
Moreover, it provides much better iteration performance for nodes, edges, and path steps.
\textsc{HashGraph} and \hl{\textsc{XG}} have similar iteration performance, but \hl{\textsc{XG}}, by virtue of its use of compressed, static data structures, requires less than half as much memory.
\hl{\textsc{ODGI}} optimized for efficient dynamic operations on graphs with higher path coverage, and in general is not as performant as other methods on this set.
...
These experiments reveal that genome graphs need not pay the computational expense of the early versions of \hl{\textsc{VG}}.
The best-performing models require an order of magnitude less memory than \hl{\textsc{VG}} while providing higher performance for basic graph access operation and element iteration.
For these reasons, \hl{\textsc{VG}} has transitioned to using these newer graph implementations. 
